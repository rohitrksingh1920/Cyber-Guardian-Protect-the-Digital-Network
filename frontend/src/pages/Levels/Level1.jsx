...
const QUESTIONS = [
  {
    ...
    wrong_tip:
      "admin123, password1, 12345678 are among the most commonly used passwords — hackers try these first.",
  },
  {
    ...
    correct_detail: {
      ...
      checks: [
        ...
        "✔ AND something you have (phone/app)",
        "✔ Attacker needs both to succeed",
      ],
      tip: "2FA blocks over 99.9% of automated credential attacks — even with a leaked password.",
    },
    wrong_tip:
      "2FA does not protect from viruses or slow internet. It specifically stops stolen passwords from being used alone.",
  },
  {
    question:
      'You get an email: "Your account was hacked — reset here." What do you do?',
    options: [
      "Click the link immediately",
      "Ignore it",
      "Open a NEW browser tab and go to the website directly",
    ...
    correct_detail: {
      ...
      checks: [
        ...
        "✔ Phishers clone login pages perfectly",
        "✔ Type the URL yourself",
      ],
      tip: 'This technique is called "direct navigation" — the gold standard for avoiding phishing.',
    },
    wrong_tip:
      "Clicking links in suspicious emails is the #1 way people get phished. Replying can also confirm your email is active.",
  },
  {
    ...
    options: [
      ...
      "Changing your password after a breach",
    ],
    correct: 2,
    correct_detail: {
      ...
      checks: [
        ...
        '✔ Called "credential stuffing" attack',
        "✔ Hackers test leaked passwords on 100s of sites",
      ],
      tip: "Password reuse is one of the top causes of account takeovers. Use unique passwords per site.",
    },
    wrong_tip:
      "Password managers and 2FA are good habits. Changing after a breach is also correct. Reuse is always the worst choice.",
  },
  {
    question: 'What is a "password salt"?',
    options: [
      "A way to make passwords longer",
      "A random value added before hashing to defeat rainbow table attacks",
    ...
    correct_detail: {
      ...
      checks: [
        ...
        "✔ Industry standard in secure databases",
      ],
      tip: 'Without salting, two users with "password123" would have identical hashes — a huge vulnerability.',
    },
    wrong_tip:
      "Salting is not encryption or length extension. It prevents attackers from using lookup tables of precomputed hashes.",
  },
];
      ...
      >
        ...
        <div
          style={{
            fontSize: 48,
            marginBottom: 12,
            filter: "drop-shadow(0 0 20px #00b8ff)",
          }}
        >
          🔐
        </div>
        ...
        >
          <p style={{ fontSize: 14, color: "var(--text)", lineHeight: 1.75 }}>
            The company's password vault is under attack.
            <br />
            <br />
            <span style={{ color: "#00e676" }}>🟢 Collect</span> strong
            passwords, password managers, MFA tokens and security keys.
            <br />
            <span style={{ color: "var(--red)" }}>🔴 Avoid</span> weak
            passwords, malware, fake login pages, keyloggers and trojans.
            <br />
            <br />
            <strong style={{ color: "var(--gold)" }}>
              Score 400 points
            </strong>{" "}
            to secure the vault and unlock the security quiz.
          </p>
        </div>
        <div
          style={{
        ...
        >
          {[
            ["🟢", "Strong Password", "+15 pts"],
            ["🟢", "Password Manager", "+20 pts"],
            ["🟢", "MFA Token", "+18 pts"],
            ["🟢", "Security Key", "+25 pts"],
            ["🔴", "Weak Password", "❌ -Life"],
            ["🔴", "Malware", "❌ -Life"],
            ["🔴", "Fake Login Page", "❌ -Life"],
            ["🔴", "Keylogger", "❌ -Life"],
          ].map(([icon, label, pts]) => (
            <div
              key={label}
            ...
            >
              ...
              <span
                style={{
                  fontFamily: "var(--font-head)",
                  fontSize: 10,
                  color: pts.startsWith("+") ? "var(--green)" : "var(--red)",
                }}
              >
                {pts}
              </span>
    ...
    >
      <div
        style={{
          ...
          textAlign: "center",
          animation: "pop .4s cubic-bezier(0.34,1.56,0.64,1)",
          boxShadow: "0 0 40px rgba(0,230,118,.15)",
        }}
      >
        ...
        >
          Password Fortress Run — Complete
        </div>
        <div
          style={{
            display: "grid",
        ...
        >
          {[
            { label: "Run Score", val: runStats.score, color: "var(--gold)" },
            {
              ...
              val: runStats.dataCollected,
              color: "var(--green)",
            },
            {
              label: "Lives Remaining",
              val: `${runStats.lives} / 3`,
              color: "var(--accent)",
            },
            {
              label: "Run Bonus",
        ...
        >
          🔐 Vault secured with run bonus! Now answer{" "}
          <strong style={{ color: "var(--text)" }}>5 security questions</strong>{" "}
    ...
    >
      {isCorrect ? (
        <>
          ...
          >
            {q.correct_detail.checks.map((c) => (
              <div key={c} style={{ fontSize: 13, color: "var(--green)" }}>
                {c}
              </div>
            ))}
          </div>
        ...
        <>
          ...
          <div style={{ fontSize: 13, color: "var(--text)", marginBottom: 6 }}>
            <span style={{ color: "var(--red)" }}>{q.options[selected]}</span> —{" "}
            {q.wrong_tip}
          </div>
          <div
            style={{
      ...
      <button
        ...
        style={{
          marginTop: 14,
          padding: "9px 22px",
          background: isCorrect ? "var(--green)" : "rgba(255,23,68,.2)",
          border: `1px solid ${isCorrect ? "var(--green)" : "var(--red)"}`,
          color: isCorrect ? "#000" : "var(--red)",
          fontFamily: "var(--font-head)",
          fontSize: 11,
          letterSpacing: 1,
          borderRadius: 6,
        ...
        >
          {[
            ...
            { label: "Run Score", val: stats.runScore, color: "var(--green)" },
            {
              ...
              val: `${stats.correct} / ${QUESTIONS.length}`,
              color: "var(--accent)",
            },
            {
              label: "Accuracy",
  ...
  return (
    <div style={{ minHeight: "100vh", animation: "fadeIn .3s ease" }}>
      ...
      <div style={{ padding: "24px 32px", maxWidth: 720, margin: "0 auto" }}>
              ...
              {q.options.map((opt, i) => (
                <button
                  ...
                  onMouseEnter={(e) => {
                    ...
                    e.currentTarget.style.background = "rgba(0,184,255,.07)";
                  }}
                  onMouseLeave={(e) => {
        ...
        {/* Rich feedback */}
        {showFeedback && q && (
          <QuizFeedback q={q} selected={selected} onNext={handleNext} />
        )}
      </div>
    </div>
  );
