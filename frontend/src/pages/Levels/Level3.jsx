...
const FILES = [
  ...
  },
  {
    ...
    malwareType: null,
    difficulty: "EASY",
    reason: "Safe — Excel spreadsheet. Safe if macros are disabled.",
    hover: {
      size: "128 KB",
  ...
  },
  {
    ...
    malwareType: "Trojan",
    difficulty: "EASY",
    reason: "MALWARE — unsigned .exe from unofficial source. Bundles a trojan.",
    hover: {
      size: "4.2 MB",
  ...
  },
  {
    ...
    difficulty: "EASY",
    reason: "MALWARE — pirated cracks are the #1 ransomware delivery method.",
    hover: {
      size: "12.6 MB",
      source: "Torrent site",
      signature: "UNSIGNED",
  ...
  },
  {
    ...
    malwareType: null,
    difficulty: "EASY",
    reason: "Safe — JPEG images cannot execute code.",
    hover: {
      size: "3.1 MB",
      source: "Phone camera",
      signature: "Verified",
  ...
  },
  {
    ...
    difficulty: "MEDIUM",
    reason:
      "MALWARE — .bat scripts execute commands. Fake updates drop ransomware.",
    hover: {
      size: "14 KB",
      source: "Email attachment",
      signature: "UNSIGNED",
  ...
  },
  {
    ...
    difficulty: "MEDIUM",
    reason:
      "MALWARE — fake double extension! Looks like PDF but is .exe spyware.",
    hover: {
      size: "2.8 MB",
      source: "Email attachment",
      signature: "UNSIGNED",
  ...
  },
  {
    ...
    difficulty: "MEDIUM",
    reason:
      "MALWARE — .scr (screensaver) files execute as programs. Classic keylogger disguise.",
    hover: {
      size: "1.9 MB",
  ...
  {
    name: "presentation_q3_results.pptx",
    icon: "📑",
    ext: "PPTX",
    cat: "Document",
    isMalware: false,
    malwareType: null,
    difficulty: "EASY",
    reason: "Safe — standard PowerPoint presentation.",
    hover: {
      size: "5.2 MB",
      source: "Company drive",
      signature: "Verified",
      created: "4 days ago",
    },
  },
  {
    name: "Zoom_Update_v5.msi",
    icon: "📦",
    ext: "MSI",
    cat: "Installer",
    isMalware: true,
    ...
    difficulty: "HARD",
    reason:
      "MALWARE — fake Zoom updater. Installs adware and steals credentials.",
    hover: {
      size: "38.4 MB",
      source: "Unverified link",
      signature: "UNSIGNED",
      created: "Today",
    },
  },
    ...
    >
      <div
        style={{
          ...
          border: `1px solid ${file.isMalware ? "var(--red)" : "var(--green)"}`,
          borderRadius: 12,
          padding: "28px 32px",
          maxWidth: 440,
          width: "90%",
      ...
      >
        ...
        {[
          ...
          {
            label: "Risk Level",
            val: file.isMalware ? "HIGH" : "LOW",
            flag: file.isMalware,
          },
          {
            label: "Source",
            val: file.hover.source,
            flag:
              file.hover.source.includes("Unknown") ||
              file.hover.source.includes("Torrent"),
          },
          {
            label: "Signature",
            val: file.hover.signature,
            flag: file.hover.signature === "UNSIGNED",
          },
          {
            label: "Downloaded",
            val: file.hover.created,
            flag: file.hover.created === "Today",
          },
        ].map((row) => (
          <div
            key={row.label}
...
export default function Level3() {
  ...
  const rank = feedback
    ? feedback.caught === malwareCount && feedback.fp === 0
      ? "A+"
      : feedback.caught === malwareCount
        ? "A"
        : feedback.caught >= malwareCount * 0.75
          ? "B"
          : "C"
    : "-";
  ...
  return (
    <div style={{ minHeight: "100vh", animation: "fadeIn .3s ease" }}>
      ...
      {analysisFile && (
        <AnalysisPopup
          file={analysisFile}
          action={() => toggle(analysisFile.name)}
          ...
          >
            ...
            >
              {[
                ...
                {
                  label: "Accuracy",
                  val: `${malwareCount > 0 ? Math.round((feedback.caught / malwareCount) * 100) : 0}%`,
                  color: "var(--gold)",
                },
                {
                  label: "Scan Time",
                  val: `${Math.min(75 - timer, 75)}s`,
                  color: "var(--accent)",
                },
            ...
            {feedback.missed > 0 && (
              ...
              >
                <strong>Missed threats:</strong>{" "}
                {files
                  .filter((f) => f.isMalware && !flagged.has(f.name))
                  .map((f) => `${f.name} (${f.malwareType})`)
                  .join(", ")}
              </div>
            )}
            <div
            ...
            >
              ...
              {[
                ...new Set(
                  files.filter((f) => f.isMalware).map((f) => f.malwareType),
                ),
              ].join(", ")}
            </div>
            <div
      ...
      <div style={{ padding: "24px 32px", maxWidth: 920, margin: "0 auto" }}>
        ...
        >
          <div>
            ...
            >
              Flag malicious files · {flagged.size} flagged · {malwareCount}{" "}
              threats hidden in {files.length} files
            </div>
          </div>
          <div
            style={{
        ...
        <div className="progress-track" style={{ marginBottom: 14 }}>
          <div
            ...
            style={{ width: `${submitted ? 100 : (timer / 75) * 100}%` }}
          />
        </div>
        ...
        >
          ⚠️ Click to flag 🚩 · Click flagged to unflag · Hover for details ·
          Right-click or tap 🔬 for full analysis · False positives cost points!
        </div>
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {!submitted && (
                      <span
                        style={{
                          ...
                          padding: "1px 7px",
                          borderRadius: 10,
                          background: `${DIFF_COLORS[file.difficulty]}15`,
                          border: `1px solid ${DIFF_COLORS[file.difficulty]}`,
                          color: DIFF_COLORS[file.difficulty],
                          fontFamily: "var(--font-head)",
                        }}
                      >
                        {file.difficulty}
                    ...
                    >
                      ...
                      <div>
                        ...
                        <span
                          style={{
                            color:
                              file.hover.signature === "UNSIGNED"
                                ? "var(--red)"
                                : "var(--green)",
                          }}
                        >
                          {file.hover.signature}
                        </span>
                      </div>
                      <div>
                        🕐 Created:{" "}
                  ...
                  {submitted && (
                    <div
                      style={{ marginTop: 5, fontSize: 11, lineHeight: 1.4 }}
                    >
                      {file.isMalware && (
                        <span
                          style={{
                            color:
                              MALWARE_COLORS[file.malwareType] || "var(--red)",
                            fontFamily: "var(--font-head)",
                            fontSize: 10,
                          }}
                        >
