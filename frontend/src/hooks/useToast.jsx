...
export const toast = {
  success: (title, msg, ms=3500) => _add?.({ type:'success', title, msg, ms }),
    ...
    <div id="toast-container">
      {toasts.map(t => (
        <div key={t.id} className={`toast toast-${t.type}${t.out ? ' out' : ''}`}>
          ...
          <div style={{ flex:1 }}>
            {t.title && <strong style={{ display:'block', marginBottom:2 }}>{t.title}</strong>}
            {t.msg   && <span   style={{ opacity:.85 }}>{t.msg}</span>}
          </div>
